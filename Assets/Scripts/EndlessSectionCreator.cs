...
public class EndlessSectionCreator : MonoBehaviour
{
    ...
    private void Update()
    {
        if(Player.transform.position.z - 0.5f > playerZPos * size.y)
        {
            gameObject.transform.position = new Vector3(0, -5f, (playerZPos + 2) * size.y);
            playerZPos++;
            if(playerZPos > 8)
            {
                Destroy(sections[0]);
    ...
    void CreateSection()
    {
        ...
        foreach(SectionTile tile in tiles)
        {
            ...
            if(rnd < 5)
            {
                GameObject temp = Instantiate(allGameObjects.coin);
                temp.transform.position = tile.tile.transform.position + new Vector3(0, 4, 0);
        ...
        if(rnd < 8)
        {
            int rnd2 = Random.Range(0, tiles.Count);
            SectionTile tile = tiles[rnd2];
            GameObject temp = Instantiate(allGameObjects.cage);
            temp.transform.position = new Vector3(tile.tile.transform.position.x, 10, tile.tile.transform.position.z);
        }
    }
    ...
    void CreatePrimitives()
    {
        ...
        foreach (SectionTile tile in tiles)
        {
            ...
            tileObject.transform.parent = tile.tile.transform;
            tileObject.transform.position = gameObject.transform.position + new Vector3(tile.pos.x, -2, tile.pos.y);
            tileObject.transform.localScale = new Vector3(1, 1.05f, 1f);
            tileObject.GetComponent<MeshRenderer>().material = groundMat;
        }
    }
    ...
    void CreateWeird()
    {
        ...
        for (int i = 0; i < Mathf.Clamp(3, 0, size.x * size.y / 2); i++)
        {
    ...
    void SpawnTile(SectionTile sectionTile, int height)
    {
        ...
        if(Random.Range(0,400) < 7)
        {
            GameObject selectedObj = allGameObjects.specialTile;
